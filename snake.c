...
int gameover = false, score = 0, snakeX[short], snakeY[short], snake_length = 1,
    foodX = 0, foodY = 0, dirX = 0, dirY = 0;
...
void setup() {
    ...
    srand(time(
        null));  // Initialize the random number generator with the current time
...
void logic() {
    ...
    if (snakeX[0] == foodX && snakeY[0] == foodY) {
        // Generate new random coordinates for food within the game field
        ...
        // If the food spawns on the snake's body, try generating new
        // coordinates again
        while (spawnOnBody) {
            foodX = 1 + rand() % (WIDTH - 2);
            foodY = 1 + rand() % (HEIGHT - 2);
            ...
            for (int i = 0; i < snake_length; i++) {
                // If the coordinates of the Apple are on the body of the snake,
